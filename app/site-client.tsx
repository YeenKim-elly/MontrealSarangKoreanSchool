...
function MenuLink({href,children}:{href:string;children:ReactNode}){
 return <DropdownMenuItem asChild><a href={href}>{children}</a></DropdownMenuItem>;
}
export default function SiteClient({initialContent,page="home",classId}:{initialContent:SiteContent;page?:SitePage;classId?:string}){
 ...
 useEffect(()=>{fetch("/api/admin/session",{cache:"no-store"}).then(r=>r.json()).then(d=>setAdmin(Boolean((d as {authenticated?:boolean}).authenticated))).catch(()=>{})},[]);
 useEffect(()=>{
 ...
 function teacherCard(t:Teacher,i:number){
  return <article className="teacher-card" key={t.id}><div className="teacher-photo">
   {t.photo?<img src={t.photo} alt={t.name+" 사진"}/>:<Users size={42} aria-label="사진 미등록"/>}
 ...
 return <>
  ...
  <a href="#page-content" className="skip-link">본문으로 이동</a>
  <header className="topbar">
   <a className="brand" href="/"><img src="/logo.png" alt="사랑한글학교 로고"/><span>{current.schoolName}</span></a>
   <nav className="desktop-nav" aria-label="주 메뉴">
    <a href="/" className={"home-link"+(page==="home"?" selected":"")} aria-current={page==="home"?"page":undefined}>홈</a>
   ...
   {admin?<button className="admin-pill active" aria-label="운영자 종료" disabled={saving||uploading} onClick={logout}><LogOut size={15}/><span>운영자 종료</span></button>:<button className="admin-pill" aria-label="운영자 모드 열기" onClick={()=>{setMessage("");setShowLogin(true)}}><LockKeyhole size={15}/><span>운영자</span></button>}
   <Sheet><SheetTrigger asChild><button type="button" className="mobile-menu-button" aria-label="전체 메뉴 열기"><MenuIcon size={23}/></button></SheetTrigger><SheetContent className="mobile-navigation" side="right"><SheetHeader><SheetTitle>{current.schoolName}</SheetTitle><SheetDescription>학교와 수업 안내</SheetDescription></SheetHeader><nav className="mobile-links" aria-label="모바일 주 메뉴"><a href="/">홈</a><div><span>학교 소개</span><a href="/about">학교 이야기</a><a href="/teachers">교장·교감과 선생님</a><a href="/schedule">수업·학기 안내</a></div><div><span>오전 한글 수업</span><a href="/classes">전체 반 보기</a>{current.classes.map(c=><a key={c.id} href={classUrl(c.id)}>{c.name}<small>{c.focus}</small></a>)}</div><div><span>오후 방과후 수업</span><a href="/programs">프로그램 알아보기</a></div></nav></SheetContent></Sheet>
   </div>
  </header>
  ...
  <div id="page-content" tabIndex={-1} className={"page-body page-"+page}>
   {page!=="home"&&<div className="breadcrumb"><a href="/">홈</a><span>/</span>{page==="class-detail"?<><a href="/classes">오전 한글반</a><span>/</span><span>{detail?.name}</span></>:<span>{{classes:"오전 한글반",teachers:"교직원 소개",programs:"오후 프로그램",about:"학교 소개",schedule:"수업·학기 안내"}[page]}</span>}</div>}
   {page==="home"&&<>
    <section className="hero home-hero">
     <div className="hero-copy"><span className="eyebrow">사랑한글학교 <span aria-hidden="true"> / </span> MONTRÉAL</span>
      ...
      <div className="hero-actions"><a className="primary-link" href="/classes">우리 반 알아보기<ArrowRight size={18}/></a><a className="home-secondary-link" href="/about">학교 이야기</a></div>
     </div>
     <figure className="home-illustration">
      <img className="book-reveal-art" src="/images/sarang-ink-hero.webp" width="1536" height="1024" fetchPriority="high" alt="초록빛 붓결과 노란 잎사귀가 펼쳐진 책 위로 흐르는 한글 배움의 일러스트"/>
     </figure>
    </section>
    <div className="home-information" aria-label="수업 안내"><div><Clock3/><span>오전 한글 수업<strong>{current.schedule}</strong></span></div><div><MapPin/><span>함께 배우는 곳<strong>{current.location}</strong></span></div><a href="/schedule">수업·학기 안내<ArrowRight size={18}/></a></div>
    <div className="home-shortcuts">{[["/about","학교 소개","소속과 교육 방향"],["/teachers","교직원 소개","교장·교감과 담당 교사"],["/programs","오후 프로그램","다양한 방과후 수업"]].map(([url,title,subtitle],i)=><a href={url} key={url}><span className="shortcut-index" aria-hidden="true">0{i+1}</span><span><strong>{title}</strong><small>{subtitle}</small></span><ArrowRight size={18}/></a>)}</div>
   </>}
   {page==="classes"&&<section className="section classes-section">
    ...
    <div className="class-grid">{current.classes.map((c,i)=><article className={"class-card tone-"+i} key={c.id}><div className="class-number">{i<4?"0"+(i+1):"성인"}</div><h2>{c.name}</h2><strong>{c.focus}</strong><p>{c.description}</p><a className="detail-link" href={classUrl(c.id)}>{admin?"상세 내용 편집":"학습 내용 자세히 보기"}<ArrowRight size={17}/></a></article>)}</div>
   </section>}
   {page==="class-detail"&&detail&&<section className="section detail-section">
    <aside className="class-sidebar" aria-label="다른 반 보기"><a href="/classes">전체 반</a>{current.classes.map(c=><a key={c.id} href={classUrl(c.id)} aria-current={c.id===classId?"page":undefined}>{c.name}<span>{c.focus}</span></a>)}</aside>
    <article className="class-detail-content"><span className="kicker">MORNING · KOREAN LANGUAGE</span>
     ...
     <div className="class-time"><Clock3 size={19}/><span>{current.schedule}<br/>{current.location}</span><a href="/schedule">운영 안내<ArrowRight size={16}/></a></div>
    </article>
   </section>}
   ...
   {page==="programs"&&<section className="section programs-section"><div className="program-intro"><span className="kicker">SUNDAY AFTERNOON</span>{admin?<>{textField("programsTitle","오후 프로그램 제목")}{textField("programsIntro","오후 프로그램 소개",true)}</>:<><h1>{pageText("programsTitle")}</h1><p>{pageText("programsIntro")}</p></>}<a className="light-link" href="/schedule">수업·학기 안내<ArrowRight size={18}/></a></div><div className="program-list">{current.programs.map((p,i)=><article id={"program-"+p.id} key={p.id}><i aria-hidden="true">{p.icon}</i>{admin?<div><Field label="프로그램 이름" value={p.name} onChange={v=>editProgram(i,{name:v})}/><Field label="프로그램 설명" value={p.description} multiline onChange={v=>editProgram(i,{description:v})}/><Field label="표시 문자" value={p.icon} onChange={v=>editProgram(i,{icon:v})}/><button className="danger" onClick={()=>{if(window.confirm(p.name+" 프로그램을 삭제할까요?"))setField("programs",draft.programs.filter((_,n)=>n!==i))}}><Trash2 size={16}/>삭제</button></div>:<div><h2>{p.name}</h2><p>{p.description}</p></div>}</article>)}{admin&&<button className="add-button" onClick={()=>setField("programs",[...draft.programs,{id:crypto.randomUUID(),name:"새 프로그램",description:"",icon:"+"}])}><Plus size={17}/>프로그램 추가</button>}</div></section>}
   {page==="about"&&<section className="section about-section"><div className="about-mark"><School/></div><div><span className="kicker">ABOUT US</span>{admin?<><Field label="학교 이름" value={current.schoolName} onChange={v=>setField("schoolName",v)}/><Field label="학교 소개 제목" value={current.aboutTitle} onChange={v=>setField("aboutTitle",v)}/><Field label="학교 소개" value={current.aboutText} multiline onChange={v=>setField("aboutText",v)}/></>:<><h1>{current.aboutTitle}</h1><p>{current.aboutText}</p></>}{info()}<a className="detail-link" href="/teachers">교장·교감과 선생님 만나기<ArrowRight size={17}/></a></div></section>}
   {page==="schedule"&&<section className="section schedule-section"><div className="section-head left"><span className="kicker">SCHOOL LIFE</span>{admin?<>{textField("scheduleTitle","운영 안내 제목")}{textField("scheduleIntro","운영 안내 소개",true)}</>:<><h1>{pageText("scheduleTitle")}</h1><p>{pageText("scheduleIntro")}</p></>}</div>{info()}<div className="schedule-links"><a href="/classes"><strong>오전 한글학교</strong><span>1–4반과 성인반 학습 안내</span><ArrowRight/></a><a href="/programs"><strong>오후 방과후 수업</strong><span>학기별 프로그램 살펴보기</span><ArrowRight/></a></div></section>}
  </div>
  <footer><img src="/logo.png" alt=""/><div><strong>{current.schoolName}</strong><span>{current.affiliation}</span></div>{admin&&page==="about"?textField("footerText","하단 문구"):<p>{pageText("footerText")}</p>}{page==="home"&&<button ref={replayRef} className="replay-intro" type="button" onClick={()=>{window.scrollTo({top:0,behavior:"instant"});setIntroRun(n=>n+1);setIntroVisible(true)}}><RotateCcw size={15}/>책 열기 다시 보기</button>}</footer>
